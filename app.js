...
if(!isMobile){
    main.addEventListener('scroll', () => {
        ...
        let scroll = (main.scrollTop / window.innerHeight) * 100;
        rounded = Math.round(scroll / 100) 
        snapTimeout = setTimeout(snap, 50);
        contentSlider.style.transform = `translate3d(${-scroll}vw, 0, 0)`
...
function mobileSnap(){
    ...
    span.className = 'flash'
    projectName.appendChild(span)
    contentSlider.style.transform = `translate3d(${-rounded * 100}vw, 0, 0)`;
}
...
if(isMobile){
    ...
    main.addEventListener('touchend', (e) => {
        ...
        if(swipe <= -20 && rounded > 0){
            rounded--
        }else if(swipe >= 20 && rounded < projectsTotal - 1){
            rounded++
        }
        mobileSnap()
    })
...
function animate(){
    for(let i = 0; i < projectsTotal; i++){
        let {left} = projects[i].getBoundingClientRect();
        projects[i].querySelector('img').style.transform = `scale(${1 + (Math.abs(left) * .0005)})`
    }
